/**
 * Course Seeding Script using existing Firestore setup
 *
 * This script uses the existing Firestore configuration from the server
 */
...
const {
  initializeFirestore,
  FirestoreUtils,
} = require("./dist/database/firestore");
...
async function seedCourses() {
  try {
    ...
    for (const course of sampleCourses) {
      const courseRef = FirestoreUtils.getCollection("courses").doc();
      batch.set(courseRef, course);
      console.log(`✅ Prepared course: ${course.name}`);
    }

    await FirestoreUtils.executeBatch(batch);
    console.log("🎉 Successfully seeded all courses!");
    console.log(`📊 Total courses created: ${sampleCourses.length}`);

    // Test the API
    console.log("\n🧪 Testing courses API...");
    const coursesSnapshot = await FirestoreUtils.getCollection("courses")
      .where("isActive", "==", true)
      .get();
