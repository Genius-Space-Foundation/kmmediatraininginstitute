...
const getFirebaseBucket = () => {
    if (!firebaseBucket) {
        try {
            ...
            firebaseBucket = firebaseStorage.bucket();
        } catch (error) {
            console.warn('Firebase not available:', error instanceof Error ? error.message : 'Unknown error');
            return null;
        }
    }
...
router.get('/courses/:courseId/modules', authenticateToken, async (req, res, next) => {
...
// Create a new module
router.post('/courses/:courseId/modules', authenticateToken, checkRole(['trainer', 'admin']), async (req, res, next) => {
    const authenticatedReq = req as AuthenticatedRequest;
    try {
        const { courseId } = req.params;
        ...
        const courseCheck = await pool.query(
            ...
            [courseId, authenticatedReq.user.id, authenticatedReq.user.role === 'admin']
        );
        ...
        next(error instanceof AppError ? error : new AppError('Failed to create module', 500));
    }
});

// Delete a module
router.delete('/courses/:courseId/modules/:moduleId', authenticateToken, checkRole(['trainer', 'admin']), async (req, res, next) => {
    const authenticatedReq = req as AuthenticatedRequest;
    try {
        ...
        const result = await pool.query(
            ...
            [moduleId, courseId, authenticatedReq.user.id, authenticatedReq.user.role === 'admin']
        );
        ...
        next(error instanceof AppError ? error : new AppError('Failed to delete module', 500));
    }
});

// Upload material to a module
router.post('/courses/:courseId/materials', authenticateToken, checkRole(['trainer', 'admin']), upload.single('file'), async (req, res, next) => {
    const authenticatedReq = req as AuthenticatedRequest;
    try {
        const { courseId } = req.params;
        ...
        // Validate file type
        const allowedTypes = ['image/', 'video/', 'application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument', 'text/'];
        ...
        const fileName = `course-materials/${courseId}/${moduleId}/${uuidv4()}-${file.originalname}`;
        const bucket = getFirebaseBucket();
        if (!bucket) {
            throw new AppError('File upload not available - Firebase not configured', 503);
        }
        ...
        next(error instanceof AppError ? error : new AppError('Failed to upload material', 500));
    }
});

// Delete material
router.delete('/courses/:courseId/materials/:materialId', authenticateToken, checkRole(['trainer', 'admin']), async (req, res, next) => {
    const authenticatedReq = req as AuthenticatedRequest;
    try {
        ...
        const bucket = getFirebaseBucket();
        if (!bucket) {
            throw new AppError('File deletion not available - Firebase not configured', 503);
        }
        const firebaseFile = bucket.file(fileName);
        ...
        next(error instanceof AppError ? error : new AppError('Failed to delete material', 500));
    }
});

// Toggle material visibility
router.patch('/courses/:courseId/materials/:materialId', authenticateToken, checkRole(['trainer', 'admin']), async (req, res, next) => {
    const authenticatedReq = req as AuthenticatedRequest;
    try {
        const { courseId, materialId } = req.params;
      ...
      RETURNING cm.*`,
            [isPublic, materialId, courseId, authenticatedReq.user.id, authenticatedReq.user.role === 'admin']
        );
        ...
        next(error instanceof AppError ? error : new AppError('Failed to update material visibility', 500));
    }
});

// Track material download
router.post('/courses/:courseId/materials/:materialId/download', authenticateToken, async (req, res, next) => {
    const authenticatedReq = req as AuthenticatedRequest;
    try {
        const { courseId, materialId } = req.params;
      ...
      RETURNING cm.*`,
        ...
        next(error instanceof AppError ? error : new AppError('Failed to track download', 500));
    }
});

// Track material view
router.post('/courses/:courseId/materials/:materialId/view', authenticateToken, async (req, res, next) => {
    const authenticatedReq = req as AuthenticatedRequest;
    try {
        const { courseId, materialId } = req.params;
      ...
      RETURNING cm.*`,
        ...
        next(error instanceof AppError ? error : new AppError('Failed to track view', 500));
    }
});
