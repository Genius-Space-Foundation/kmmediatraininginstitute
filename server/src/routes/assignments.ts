...
router.post('/', authenticateToken, async (req: AuthRequest, res) => {
    try {
        if (!req.user || req.user.role !== 'trainer') {
            return res.status(403).json({ success: false, message: 'Only trainers can create assignments' });
        }
...
router.post('/submissions/:id/grade', authenticateToken, async (req: AuthRequest, res) => {
    try {
        if (!req.user || req.user.role !== 'trainer') {
            return res.status(403).json({ success: false, message: 'Only trainers can grade submissions' });
        }
...
router.get('/:id/submissions', authenticateToken, async (req: AuthRequest, res) => {
    try {
        if (!req.user || req.user.role !== 'trainer') {
            return res.status(403).json({ success: false, message: 'Only trainers can view all submissions' });
        }
...
router.get('/my-submissions', authenticateToken, async (req: AuthRequest, res) => {
    try {
        if (!req.user || req.user.role !== 'user') {
            return res.status(403).json({ success: false, message: 'Only students can view their submissions' });
        }
